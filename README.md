...
Hackathon submission repo for **DARTS**, built for Redrob AI's India Runs Track 1: Intelligent Candidate Ranking.

DARTS is a deterministic, explainable candidate-ranking system designed to rank candidates by real role relevance instead of keyword overlap. It evaluates career evidence, skill strength, education and location fit, behavioral readiness, and suspicious-profile signals to produce a trusted top-100 shortlist.

The file to upload to the portal is `submission/official_submission.csv`. `submission/top_100.csv` has been removed because it was an earlier internal-review format and is not a valid submission.

## Objective

Rank the top 100 candidates for Redrob's Senior AI Engineer, Founding Team role.

The JD targets a hands-on production ML/AI engineer with retrieval, ranking, embeddings, evaluation, and product-shipping experience. DARTS translates those requirements into a structured scoring system that rewards relevant AI/ML career history, strong technical skill evidence, product-facing engineering exposure, and reliable behavioral signals.
...
Company and title tiering is manual because Phase 0 found a small universe: 63 unique career-history companies and 48 unique career-history titles. Manual classification is stronger than an LLM pass here because every company and title can be reviewed directly.
...
* The 60-day notice-period threshold was chosen over alternatives because `validation/results/threshold_results.json` showed top-100 overlap dropping to 85.0% at 45 days, while 75 days held at 100% overlap. The 60-day threshold is closest to typical Indian notice-period norms within the range that does not destabilize the ranking.
...
Score is normalized to a 0-1 range by dividing by the behavioral modifier's defined maximum value of 1.30.

Honeypot exclusion runs before final ranking. A candidate is excluded only when at least two independent suspicious-profile flags trigger.
...
records_ranked_after_honeypot: 99984
...
Open `notebooks/sandbox_demo.ipynb` in Google Colab and run all cells.

The notebook clones this public repo, loads only `data/sample/sample_candidates.json`, converts at most 100 sample candidates into a temporary JSONL file, reuses `phase_b.rank.rank_candidates`, and writes `/content/sandbox_official_submission.csv` with the official `candidate_id,rank,score,reasoning` schema.

It does not embed, fetch, or require the full private `data/raw/candidates.jsonl` dataset.
...
The GitHub repository may still use the challenge-oriented name `IndiaRunATS`, but the product name for the solution is **DARTS**. All presentation, README, and submission-facing material should refer to the system as **DARTS** for consistency.
